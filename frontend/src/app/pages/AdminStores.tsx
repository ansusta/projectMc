...
import { Store, CheckCircle, XCircle, Info, ExternalLink } from 'lucide-react';
...
export const AdminStores = () => {
  ...
  const handleValidation = async (storeId: string, status: 'actif' | 'suspendu') => {
    const confirmMsg = status === 'actif' ? t('adminStores.validateOpen') : t('adminStores.rejectSuspend');
    if (!window.confirm(confirmMsg)) return;

    try {
      await adminService.validateStore(storeId, status);
      setStores(prev => prev.filter(s => s.id !== storeId));
      toast.success(status === 'actif' ? t('adminStores.approveSuccess') : t('adminStores.suspendSuccess'));
    } catch (error) {
      toast.error(t('adminStores.validationError'));
    }
  };
      ...
      <div className="max-w-6xl mx-auto space-y-5 sm:space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
          ...
          <div className="text-left sm:text-right">
             <span className="text-primary/60 text-xs tracking-widest">{stores.length} {t('adminStores.unitsPending')}</span>
          </div>
        </div>
              ...
              <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-cyan-600/60 uppercase italic flex items-center gap-1 font-mono">
                        <Info size={10} /> {t('adminStores.createdOn')} {new Date(store.date_creation).toLocaleDateString(i18n.language)}
                    </p>
                  </div>
                </div>
                ...
                <div className="mb-8 h-20 overflow-y-auto pr-2 custom-scrollbar">
                    <p className="text-xs text-gray-400 leading-relaxed italic opacity-70">
                        {store.description || t('adminStores.noDescription')}
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-cyan-500/10">
                    ...
                    >
                        <CheckCircle size={14} /> {t('adminStores.approveUnit')}
                    </button>
                    <button 
        ...
        {stores.length === 0 && (
          <div className="text-center py-32 bg-card/10 border border-dashed border-cyan-500/20 rounded-2xl shadow-sm">
            ...
            <h3 className="text-cyan-600/40 text-sm tracking-[0.4em] uppercase italic font-mono">{t('adminStores.protocolUpToDate')}</h3>
            <p className="text-cyan-600/20 text-[10px] mt-2 uppercase font-mono">{t('adminStores.noRequests')}</p>
          </div>
        )}
      </div>
    </div>
